...
class DetectFeatLmdb(object):
    ...
    def __getitem__(self, file_name):
        ...
        if self.compress:
            ...
            img_dump = msgpack.loads(dump, raw=False)
        base_img_feat = torch.tensor(img_dump['base_features'][:nbb, :]).float()
        try:
            fine_img_feat = torch.tensor(img_dump['fine_features'][:nbb, :]).float()
        except:
            fine_img_feat = base_img_feat
            self.missing.append(file_name)
            print(file_name)
        img_bb = torch.tensor(img_dump['norm_bb'][:nbb, :]).float()
        return base_img_feat, fine_img_feat, img_bb
...
class DetectFeatTxtTokDataset(Dataset):
    def __init__(self, cate2idx_path, txt_db, img_db):
        ...
        # lens = [txt_db.id2len[idx] for idx in self.ids] #这个是不算nbb的
    ...
    def __getitem__(self, i):
        id_ = self.ids[i]
        input_ids = self.txt_db[id_] #直接返回list
        return input_ids

    def _get_img_feat(self, fname):
        base_img_feat, fine_img_feat, bb = self.img_db[fname]
        img_bb = torch.cat([bb, bb[:, 4:5]*bb[:, 5:]], dim=-1)
        num_bb = base_img_feat.size(0)
        return base_img_feat, fine_img_feat, img_bb, num_bb
...
def pad_tensors(tensors, lens=None, pad=0):
    ...
    # print("tensors shape", len(tensors))
...
#use gathe_index to align with attn_mask
...
class ClassDataset(DetectFeatTxtTokDataset):
    ...
    def __init__(self, cate2idx_path, txt_db, img_db):
        ...
        idx2user_path = 'Public_Dataset/Data/idx/idx2user.json'
        with open(idx2user_path,'r') as f:
            self.idx2user = json.load(f)
        f.close()
    ...
    def __getitem__(self, i):
        ...
        # img input
        base_img_feat, fine_img_feat, img_pos_feat, num_bb = self._get_img_feat(idx)
        ...
        img_feat = torch.cat([base_img_feat.unsqueeze(0), fine_img_feat.unsqueeze(0)], dim=0) #[2,l,d] no bs before collate
        userid = int(self.idx2user[idx])

        return userid, input_ids, img_feat, img_pos_feat, attn_masks, label, idx


def class_collate(inputs):
    (userids, input_ids, img_feats, img_pos_feats, attn_masks, labels, idxs
     ) = map(list, unzip(inputs))

    txt_lens = [i.size(0) for i in input_ids]

    input_ids = pad_sequence(input_ids, batch_first=True, padding_value=0)
    txt_mask = _compute_mask(txt_lens, max(txt_lens))
    position_ids = torch.arange(0, input_ids.size(1), dtype=torch.long
                                ).unsqueeze(0)

    num_bbs = [f.size(1) for f in img_feats]
    # img_feat = pad_tensors(img_feats, num_bbs) #[[2,l1,dim], [2,l2,dim]]
    base_img_feat = [feat[0,:] for feat in img_feats]
    base_img_feat = pad_tensors(base_img_feat,num_bbs) #[b,l,d]

    fine_img_feat = [feat[1,:] for feat in img_feats]
    fine_img_feat = pad_tensors(fine_img_feat,num_bbs) #[b,l,d]

    img_feat = torch.cat([base_img_feat.unsqueeze(1), fine_img_feat.unsqueeze(1)], dim=1) #[b,2,l,d]

    img_mask = _compute_mask(num_bbs, max(num_bbs))
    img_pos_feat = pad_tensors(img_pos_feats, num_bbs)
    labels = torch.from_numpy(np.array(labels))
    # idxs = torch.from_numpy(np.array(idxs))
    userid = torch.from_numpy(np.array(userids))
...
def _compute_mask(lens, max_len):
    pad = torch.zeros(len(lens), max_len, dtype=torch.uint8)
    for i, l in enumerate(lens):
        pad.data[i, :l].fill_(1)
    return pad
